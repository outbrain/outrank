...
def generate_random_matrix(num_features, size=2000):
    # random int matrix (categorical)
    sample = np.random.randint(10, 100, size=(size, num_features))

    target = sample[:, 30] + sample[:, 50] + sample[:, 20]
...
if __name__ == '__main__':
    ...
    if args.output_df_name is not None:
        ...
        if (
            rankings.iloc[1]['Feature'] != 'f20-(90; 100)'
            and rankings.iloc[2]['Feature'] != 'f50-(90; 100)'
            and rankings.iloc[3]['Feature'] != 'f30-(90; 100)'
        ):
            raise Exception(
            ...
            logger.info(
                f'Identified the appropriate feature in the haystack ({rankings.iloc[1:4].Feature})',
            )
